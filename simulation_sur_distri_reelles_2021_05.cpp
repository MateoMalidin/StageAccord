...
void nbfois_unplusnbGgroupe(int nb, int nbG, int RefIni[], int nbIt, int nbC, int TE[], float TEIt[][MAXCL], int nbA, float tauxErparAnnot, float sigmatauxEr, int choix1, int choix2, float & moymtauxErRef, float & moysigmatauxErRef, float & moymtauxconf, float & moyalpha, float & moyalphaconf, float & moykappa, float & moycos_uniforme, float & moydistri_hasard, float & moydistance_distri_hasard, float & distancemoytaux_distri_hasard) {
  float mtauxErRef, sigmatauxErRef, mtauxconf, alpha, alphaconf, kappa, cos_uniforme, distri_hasard, distance_distri_hasard, distancetaux_distri_hasard;
  moymtauxErRef = 0;
  moysigmatauxErRef = 0;
  moymtauxconf = 0;
  moyalpha = 0;
  ...
  moykappa = 0;
  moydistri_hasard = 0;
  moydistance_distri_hasard = 0;
  for (int i = 0; i < nb; i++) {
    //cout << "\n unplusnbGgroupe, i=" << i << endl;
    unplusnbGgroupe(nbG, RefIni, nbIt,nbC, TE, TEIt, nbA, tauxErparAnnot, sigmatauxEr, choix1, choix2, mtauxErRef, sigmatauxErRef, mtauxconf, alpha, alphaconf, kappa, cos_uniforme, distri_hasard, distance_distri_hasard, distancetaux_distri_hasard);
...
int main(int n, char * param[]) {
  ...
  while (c < size && args[c] != "") {
    ...
    /*
    transpose(T, TT, nbIt, nbAR);
    float kappa = kappaAP(nbIt, nbC, nbAR, TT);
  ...
  string nomfich = "./res/stats.txt";
  ofstream file(nomfich.c_str());
  for (int i = 0; i < size; i++) {
    file << args[i] << " : " << stats[i] << endl;
  }*/
  return 0;
}

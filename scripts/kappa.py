# programme de visualisation des résultats simulés (kappa de Fleiss)
...
for c in corpus: # pour chaque corpus
    file = open("kappa_" + c + ".csv", "r") # ouverture du fichier de resultats
    lines = file.readlines() # lecture du fichier
...
plt.title("kappa -> taux -> 5 classes -> 5 annotateurs")
plt.xlabel("kappa")
plt.ylabel("taux")
plt.xlim([0.2, 0.9])
plt.ylim([0.0, 0.4])
